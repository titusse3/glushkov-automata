module Exp(Exp(..), followE, linearisation, alphabet, firstE, lastE, glushkov) where
...
alphabet (Plus e e') = Set.union (alphabet e) (alphabet e')
alphabet (Star e) = alphabet e
alphabet (Point e e') = Set.union (alphabet e) (alphabet e')
...
firstE (Point e e') = if isNull e then
    Set.union (firstE e) (firstE e') else firstE e
...
lastE (Point e e') = if isNull e' then
    Set.union (lastE e) (lastE e') else lastE e'
...
followE (Plus e e') s
  ...
  | Map.member s (posE e') = followE e' s
  | otherwise = Set.empty
followE (Star e) s = if Set.member s (lastE e) then
  Set.union (firstE e) (followE e s) else firstE e
followE (Point e e') s
  | Map.member s (posE e') = followE e' s
  | Map.member s (posE e) = if Set.member s (lastE e) then
      Set.union f (firstE e')
    else
      f
  | otherwise = Set.empty
  where
...
glushkov e = NFA sigma' q i f trans
  where sigma' = alphabet e
        i = Set.singleton 0
        f = if isNull linear then Set.insert 0 l else l
        ...
        fist = firstE linear
        trans s t = z
          where sFollow = if s == 0 then fist else followE linear s
